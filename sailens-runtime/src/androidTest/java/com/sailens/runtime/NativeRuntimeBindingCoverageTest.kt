 ...
 *
 * ```
 * $ llvm-nm -D --defined-only libsailens_runtime.so | grep -E 'Java_|JNI_OnLoad'
 ...
 *    calls all of them.
 *
 * Needs a physical arm64 device: `./gradlew.bat :sailens-runtime:connectedDebugAndroidTest`.
 */
@RunWith(AndroidJUnit4::class)
class NativeRuntimeBindingCoverageTest {

    /**
     * The Kotlin-side copy of the RegisterNatives table in
     * `sailens-runtime/src/main/cpp/sailens_runtime_jni.cpp`.
     *
    ...
    fun nativeLibraryLoads() {
        assertTrue(
            "libsailens_runtime.so did not load. JNI_OnLoad returns JNI_ERR when a class, method " +
                "name or signature in the RegisterNatives table does not match the Kotlin " +
                "declarations; logcat tag SailensRuntimeJni names the offending class. Cause: " +
                "${NativeRuntimeLibrary.loadError}",
            NativeRuntimeLibrary.isAvailable,
        )
    }
    ...
    fun declaredNativeMethodsMatchTheRegistrationTable() {
        ...
        assertEquals(
            "A changed declaration must be mirrored in the RegisterNatives table in " +
                "sailens-runtime/src/main/cpp/sailens_runtime_jni.cpp and in expectedBindings here.",
                ...
                if (cause is UnsatisfiedLinkError) {
                    fail(
                        "${binding.key} is declared in Kotlin but not registered in " +
                            "sailens-runtime/src/main/cpp/sailens_runtime_jni.cpp: $cause",
                    )
                }
