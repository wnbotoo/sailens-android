...
- 每个会话一个目录，放在应用内部 `files/captures/` 下，JSONL + 图像文件；手动导出；可在调试 UI
  里列出和删除；release 版不包含。采集数据里有人脸和地点，不会自动离开设备。
...
**手机高度**：`PhoneGeometrySettings` 来自 shell 持有的存储（运行参数预设只提供 1.3 m 初始值）。启
用 M3a 要求高度来源为 `PRESET` 或 `CALIBRATED`；只有初始值时绝不改变距离分级。
...
产出决策记录，不产出生产代码。候选方案都用 M0 采集的现场数据评估：前向轴已知的刚性固定（胸前背
带/挂绳），对照实际行走方向验证；基于计步的航位推算（需要先定 `ACTIVITY_RECOGNITION` 权限）；从相
邻帧估计的视觉运动（例如地平面光流——M3 已经给出了地平面）。结论用于正式方向指引门槛和 M6 的滚动
占据。
