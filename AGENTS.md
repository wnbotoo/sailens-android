...
- Every `SceneEvent.messageKey` comes from `SceneEventMessageKeys` (sailens-guidance) and is mapped to a string at compile time in `SceneEventStrings` (sailens-shell), present in both `values/` and `values-zh/`. Never look announcement strings up by name (`getIdentifier`): release resource shrinking removes them. `SceneEventStringsTest` and `scripts/verify-release-strings.py` guard this.
